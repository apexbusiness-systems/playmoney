...
async function run() {
  ...
  try {
    ...
    const { error: loaErr } = await supabaseAdmin.from("loa_tokens").insert([
      { owner_id: idA, recovery_id: "test-rec-a", avenue: "test", merchant: "test", max_amount_cents: 1000, signed_by: "A", signature_method: "test", signature_statement: "test", consent_electronic: true, expires_at: expiresAt, idempotency_key: "tok_a" },
      { owner_id: idB, recovery_id: "test-rec-b", avenue: "test", merchant: "test", max_amount_cents: 1000, signed_by: "B", signature_method: "test", signature_statement: "test", consent_electronic: true, expires_at: expiresAt, idempotency_key: "tok_b" },
    ]);
    ...
    // 1. Anon access
    const { data: anonProfiles } = await supabaseAnon.from("profiles").select("*").eq("jurisdiction_province", `Test AB ${timestamp}`);
    if (anonProfiles && anonProfiles.length > 0) throw new Error("Anon could read profiles!");
    const { data: anonAudit } = await supabaseAnon.from("audit_log").select("*").eq("action", "RLS_TEST");
    ...
    if (!aProfiles || aProfiles.length === 0) throw new Error("Owner A could not read own profile!");
    ...
    if (aReadsBLoa && aReadsBLoa.length > 0) throw new Error("Owner A could read Owner B's loa_tokens!");

    // 4. Cross-user update failure
    const { error: bUpdatesA } = await clientB.from("profiles").update({ display_name: "Hacked" }).eq("id", idA);
    // Supposed to fail silently or return error but update 0 rows
    const { data: aAfterHack } = await clientA.from("profiles").select("display_name").eq("id", idA).single();
    if (aAfterHack?.display_name === "Hacked") throw new Error("Owner B was able to update Owner A's profile!");

    console.log("✅ Owner isolation proven (A reads only A, B reads only B, cross-reads/writes fail).");
