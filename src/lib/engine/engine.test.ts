...
const tx = (over: Partial<BankTransaction> & Pick<BankTransaction, "id" | "postedAt" | "amountCents">): BankTransaction => ({
  merchant: "Acme",
  description: "",
  ...over,
});
...
describe("P3 · SituationModel: pure, deterministic detection", () => {
  it("detects a duplicate (double) charge from two equal same-window txns", () => {
    const txns: BankTransaction[] = [
      tx({ id: "t1", merchant: "Uber", amountCents: 5140, postedAt: "2026-06-10T10:00:00.000Z", description: "Ride" }),
      tx({ id: "t2", merchant: "Uber", amountCents: 5140, postedAt: "2026-06-10T10:05:00.000Z", description: "Ride" }),
    ];
  ...
  it("detects a reversible fee by keyword → fee_reversal", () => {
    const sits = deriveSituations([
      tx({ id: "f1", merchant: "Chase", amountCents: 3500, postedAt: "2026-06-01T00:00:00.000Z", description: "Overdraft fee" }),
    ]);
  ...
  it("detects a recurring subscription from 3+ equal charges", () => {
    const sits = deriveSituations([
      tx({ id: "s1", merchant: "Spotify", amountCents: 1199, postedAt: "2026-03-01T00:00:00.000Z", description: "Premium" }),
      tx({ id: "s2", merchant: "Spotify", amountCents: 1199, postedAt: "2026-04-01T00:00:00.000Z", description: "Premium" }),
      tx({ id: "s3", merchant: "Spotify", amountCents: 1199, postedAt: "2026-05-01T00:00:00.000Z", description: "Premium" }),
    ]);
    expect(sits).toHaveLength(1);
  ...
  it("is deterministic and consumes each txn at most once", () => {
    const txns: BankTransaction[] = [
      tx({ id: "d1", merchant: "Uber", amountCents: 5140, postedAt: "2026-06-10T10:00:00.000Z", description: "Ride" }),
      tx({ id: "d2", merchant: "Uber", amountCents: 5140, postedAt: "2026-06-10T10:05:00.000Z", description: "Ride" }),
      tx({ id: "x1", merchant: "Chase", amountCents: 3500, postedAt: "2026-06-01T00:00:00.000Z", description: "NSF fee" }),
    ];
  ...
  it("returns nothing for an unremarkable single charge", () => {
    expect(
      deriveSituations([
        tx({ id: "n1", merchant: "Grocer", amountCents: 4210, postedAt: "2026-06-01T00:00:00.000Z", description: "Groceries" }),
      ]),
    ).toEqual([]);
  });
});
...
describe("P3 · AvenueRouter: never routes to a disabled avenue", () => {
  ...
  it("routeSituation defers to the situation's problemType", () => {
    const res = routeSituation({
      situation: { id: "sit_x", merchant: "Uber", detectedAt: "2026-06-10T10:00:00.000Z", summary: "dup" },
