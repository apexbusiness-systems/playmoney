...
export const Route = createFileRoute("/privacy")({
  head: () => ({
    meta: [
      { title: "Privacy Policy — PlayMoney" },
      {
        name: "description",
        content: "PlayMoney Privacy Policy and data handling disclosures.",
      },
    ],
  }),
        ...
        <div className="mx-auto max-w-3xl">
          <div className="border-b border-border-l pb-8">
            <p className="eyebrow text-ink-muted">
              {t("landing.footer.legal")} · v{PRIVACY_LATEST.version}
            </p>
            <h1 className="h2-display mt-2">{t("legal.title.privacy")}</h1>
            <div className="mt-4 flex flex-wrap items-center gap-4 text-sm text-ink-muted">
              <span>
                {t("legal.meta.updatedAt")}:{" "}
                <strong className="font-semibold text-ink">{PRIVACY_LATEST.updatedAt}</strong>
              </span>
              <span>·</span>
              <span>
                {t("legal.meta.version")}:{" "}
                <strong className="font-semibold text-ink">{PRIVACY_LATEST.version}</strong>
              </span>
              <span>·</span>
