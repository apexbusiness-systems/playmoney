      ...
      <div className="mt-10 grid gap-10 lg:grid-cols-[3fr_2fr]">
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-sand text-ink-muted">
                <tr>
                  ...
                  <th className="px-5 py-3 font-semibold text-right">Net to you</th>
              ...
              <tbody>
                {recs.data?.map((r) => (
                  <tr key={r.id} className="border-t border-border-l">
                    ...
                    <td className="px-5 py-4 text-right font-display tabular">{formatMoney(r.userNet)}</td>
        ...
        <div>
          ...
          <div className="mt-4 space-y-2">
            {fees.data?.map((f) => (
              <div key={f.id} className="flex items-center justify-between rounded-[14px] border border-border-l bg-card px-5 py-3 text-sm">
                <span className="text-ink-muted">{new Date(f.ts).toLocaleDateString()}</span>
                <span className="font-display tabular font-semibold text-evergreen">{formatMoney(f.feeAmount)}</span>
              </div>
            ))}
            {fees.data?.length === 0 && (
