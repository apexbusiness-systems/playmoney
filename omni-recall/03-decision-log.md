...
### 2026-06-22 · D-016 · OmniPort Connector

Installed the APEX-OmniHub OmniPort sidecar connector.

- **New:** `src/lib/omniport/{types,auth,commands,snapshot,omniport.test}.ts` ·
  `src/lib/api/omniport.functions.ts` · `src/routes/api/omniport/{sync,health,command}.tsx` ·
  `supabase/migrations/0010_omniport.sql` · `OMNIPORT.md`.
- **Edited:** `AppShell.tsx` (desktop-only branding, opacity 0.35, pointer-events:none) ·
  `.env.example` (+OMNIPORT_ENABLED, +OMNIPORT_SECRET).
- `OMNIPORT_ENABLED=false` by default — connector is fully offline until enabled via
  `wrangler secret`. Every `/api/omniport/*` route returns 503 when disabled (the enabled
  guard is the first line of each handler, before any auth/parse/DB call).
- **Zero compliance coupling.** No imports from `omniport/` into `compliance/`; nothing in
  `omniport/` imports or calls `assertLiveAllowed()`. Touches zero Cents / payout refs /
  fund-holding surface — invariant #1 untouched. The two new tables are service-role-only
  (RLS enabled, no policies → anon/authenticated denied), matching `go_live_attestations`.
- **Auth:** HMAC-SHA256 over the raw body via the Web Crypto API (`crypto.subtle`), native in
  Workers and the Node test runtime — no Node `crypto`, no test stub. Bad signature → 401,
  bad schema → 400 (Zod), disabled → 503. Body/signature never logged.
- **Receipts are DB-derived:** SET_FEATURE_FLAG → DB `updated_at`; BROADCAST_NOTICE /
  REFRESH_CONFIG → DB-generated row `id`; HEALTH_PING → snapshot `checkedAt`. No receipt is
  constructed before its write resolves.

**YELLOW-lane deviations from the brief (both proven against the installed code):**
1. Route loaders use `getRequest()` from `@tanstack/react-start/server` instead of
   `loader: async ({ request })` — `LoaderFnContext` (router-core v1.168) exposes no `request`
   field, so the brief's destructure would not type-check. `getRequest()` is the supported
   server accessor. routeTree.gen.ts is regenerated by the router vite plugin on build.
2. `0010_omniport.sql` follows the ACTUAL repo migration pattern (`create table if not exists`
   + RLS; runner records the version) rather than the brief's literal `DO $$ ... migration = ...`
   self-guard — `private.schema_migrations`'s column is `version`, not `migration`, and the
   runner (`scripts/db/migrate.ts`) inserts the bookkeeping row itself, so a self-insert would
   be wrong/duplicate. `command_log.receipt` defaults to `''` then is stamped with the row id.

**Verified:** see post-build run appended to this entry.

**Verified (D-016):** `bun run typecheck` clean · `bun run lint` exit 0 ·
`bun run test` = **302 passing / 32 files** (+16 new: omniport auth/snapshot/commands/schema)
